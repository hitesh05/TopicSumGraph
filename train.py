...
def main():
    ...
    # Important settings
    parser.add_argument('--model', type=str, default='TSG', help='model structure[HSG|HDSG]')
    ...
    parser.add_argument('--word_emb_dim', type=int, default=768, help='Word embedding size [default: 768 of bert-base]')
    ...
    if hps.model == "TSG":
        model = TopicSumGraph(hps, vocab_size=vocab.size())
        logger.info("[MODEL] HeterSumGraph ")
        dataset = ExampleSet(DATA_FILE, vocab, hps.doc_max_timesteps, hps.sent_max_len, FILTER_WORD, train_w2s_path, num_topics=hps.num_topics)
        ...
        del dataset
        valid_dataset = ExampleSet(VALID_FILE, vocab, hps.doc_max_timesteps, hps.sent_max_len, FILTER_WORD, val_w2s_path, num_topics=hps.num_topics)
